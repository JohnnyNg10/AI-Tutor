...
#### 水滴组件
```http
GET /mastery/component/water-drop
```
...
#### 组件配置
```http
GET /mastery/component/config?component_type=ring
```
...
### 8.1 圆环组件
- **端点**: `GET /mastery/component/ring`
- **功能**: 返回圆环组件所需的完整数据
- **颜色**: 根据P(L)动态返回红/黄/绿
- **动画**: 支持1秒动画时长配置

### 8.2 水滴组件
- **端点**: `GET /mastery/component/water-drop`
- **功能**: 返回水滴组件所需的完整数据
- **水位**: 0-100，基于P(L)计算
- **波浪**: 动态波浪高度（掌握度越高越平缓）
- **动画**: 支持波浪动画
...
### 8.3 组件配置
- **端点**: `GET /mastery/component/config`
- **功能**: 返回组件的详细配置参数
- **类型**: 支持ring/water_drop两种组件
...
git commit -m "feat: Implement mastery visualization with ring/water-drop components (Row 2)

- Add P(L) to level conversion (weak/learning/mastered)
- Add P(L) to color mapping (red/yellow/green) - 硬指标
- Add water level calculation (0-100)
- Add ring component API (/mastery/component/ring)
- Add water-drop component API (/mastery/component/water-drop)
- Add component config API for frontend integration
- Add topic progress calculation
- Update implementation documentation

对应飞书需求池行号2: 弃用传统进度条，采用圆环或水滴组件
颜色联动硬指标:
- P(L) < 0.5: 红色 #FF4D4F (危险区)
- 0.5 <= P(L) < 0.8: 黄色 #FAAD14 (过渡区)
- P(L) >= 0.8: 绿色 #52C41A (掌握区)"
```
